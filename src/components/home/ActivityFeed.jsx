...
export default function ActivityFeed({ trucks, orders = [] }) {
  ...
  liveTrucks.slice(0, 2).forEach(t => {
    events.push({
      ...
      text: <><span style={{ color:'var(--cc-on-cream)', fontWeight: 700 }}>{t.name}</span> just went <span style={{ color:'var(--cc-warm-red)', fontWeight: 700 }}>LIVE </span></>,
  ...
  openTrucks.slice(0, 2).forEach((t, i) => {
    events.push({
      id: `open-${t.id}`,
      icon: ShoppingBag,
      iconColor:'var(--cc-teal-on-cream)',
      iconBg:'rgba(var(--cc-accent-rgb),0.1)',
      text: <><span style={{ color:'var(--cc-on-cream)', fontWeight: 700 }}>{t.name}</span> is <span style={{ color:'var(--cc-teal-on-cream)', fontWeight: 700 }}>Open Now</span></>,
  ...
  if (trucks.length > 0) {
    ...
    if (top) events.push({
      ...
      text: <><span style={{ color:'var(--cc-on-cream)', fontWeight: 700 }}>{top.name}</span> is trending </>,
  ...
  return (
    <div className="px-5 mt-6">
      ...
      <div className="flex flex-col gap-0 rounded-2xl overflow-hidden" style={{ background:'var(--cc-cream-2)', border:'1px solid rgba(var(--cc-cream-line-rgb),0.2)'}}>
        {events.slice(0, 4).map((ev, i) => {
          const Icon = ev.icon;
          return (
            <Link key={ev.id} to={ev.href}>
              <div className="flex items-center gap-3 px-4 py-3.5 transition-colors active:bg-white/5" style={{ borderBottom: i < events.slice(0, 4).length - 1 ?'1px solid rgba(var(--cc-cream-line-rgb),0.12)':'none'}}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm leading-snug" style={{ color:'var(--cc-on-cream-muted)'}}>{ev.text}</p>
