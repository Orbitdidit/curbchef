      ...
      >
          ...
          <div className="absolute top-2 right-2">
            {truck.is_live ? (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold font-mono"
                style={{ background: 'rgba(var(--cc-warm-red-rgb),0.9)', color: 'white' }}>
        ...
        <div className="px-3 pt-2 pb-3">
          ...
          <div className="flex items-center justify-between mt-1.5">
            ...
            {dist && (
              <div className="flex items-center gap-0.5">
                <MapPin className="w-2.5 h-2.5" style={{ color: 'var(--cc-ink-faint)' }} />
                <span className="text-[10px] font-mono" style={{ color: 'var(--cc-ink-faint)' }}>{dist}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {badge && badge !== 'live' && (
            <span
              ...
              style={{ background: 'rgba(var(--cc-warm-red-rgb),0.12)', color: 'var(--cc-warm-red)', border: '1px solid rgba(var(--cc-warm-red-rgb),0.2)' }}
            >
              {badge}
            </span>
          )}
        </div>
        <Link to={seeAllHref} className="flex items-center gap-0.5 text-xs font-semibold" style={{ color: 'var(--cc-accent-2)' }}>
