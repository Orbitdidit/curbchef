  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center"
      ...
      <div className="w-full max-w-lg rounded-t-3xl flex flex-col"
        style={{ background: 'var(--cc-bg-1)', border:'1px solid rgba(var(--cc-accent-rgb),0.2)', maxHeight:'75vh' }}
        onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid rgba(var(--cc-line-rgb),0.3)' }}>
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user'?'justify-end':'justify-start'}`}>
              <div className="max-w-[85%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap"
                style={m.role === 'user'? { background:'rgba(var(--cc-accent-rgb),0.12)', color:'var(--cc-ink)', border:'1px solid rgba(var(--cc-accent-rgb),0.2)'}
                  : { background:'#1e2a2c', color:'var(--cc-ink-dim)' }}>
                {m.content}
              </div>
            </div>
          ))}
          {loading && (
            <div className="flex justify-start">
              <div className="px-4 py-2.5 rounded-2xl" style={{ background: '#1e2a2c' }}>
                <Loader2 className="w-4 h-4 animate-spin" style={{ color: 'var(--cc-accent)' }} />
              </div>
            </div>
          )}
          <div ref={bottomRef} />
        ...
        <div className="px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3"
          style={{ borderTop: '1px solid rgba(var(--cc-line-rgb),0.3)' }}>
          <div className="flex gap-2 items-end rounded-2xl px-4 py-3"
            style={{ background: '#1e2a2c', border:'1px solid rgba(var(--cc-line-rgb),0.4)' }}>
            <textarea
              rows={1}
              value={input}
            ...
            <button onClick={sendMessage} disabled={!input.trim() || loading}
              ...
              style={{ background: 'linear-gradient(135deg,var(--cc-accent),var(--cc-accent-3))', opacity: !input.trim() || loading ? 0.4 : 1 }}>
              <Send className="w-3.5 h-3.5" style={{ color: 'var(--cc-accent-deep)' }} />
            </button>
          </div>
        </div>
      </div>
