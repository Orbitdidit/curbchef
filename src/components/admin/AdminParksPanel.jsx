      ...
      <div className="w-full max-w-lg rounded-t-3xl sm:rounded-3xl p-5 overflow-y-auto max-h-[90vh]"
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center"
            style={{ background: 'rgba(var(--cc-line-rgb),0.3)', color: 'var(--cc-ink-dim)' }}>✕</button>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {[
            ...
            <div key={key}>
              <label className="text-[10px] font-bold block mb-1" style={{ color: 'var(--cc-ink-dim)' }}>{label}</label>
              <input
                type={type}
                value={form[key] || ''}
                onChange={onChange || (e => setForm(f => ({ ...f, [key]: type === 'number' ? parseFloat(e.target.value) || 0 : e.target.value })))}
                className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                style={{ background: 'var(--cc-bg-0)', border: '1px solid rgba(var(--cc-line-rgb),0.3)', color: 'var(--cc-ink)' }}
              />
            </div>
          ))}

          <div>
            <label className="text-[10px] font-bold block mb-1" style={{ color: 'var(--cc-ink-dim)' }}>Description</label>
            <textarea
          ...
          {/* Amenities */}
          <div>
            <label className="text-[10px] font-bold block mb-1" style={{ color: 'var(--cc-ink-dim)' }}>Amenities</label>
            <div className="flex gap-2 mb-2">
              ...
              <button onClick={addAmenity} className="px-3 py-2 rounded-xl text-sm font-bold"
                style={{ background: 'rgba(0,245,212,0.15)', color: 'var(--cc-accent-2)' }}>Add</button>
            </div>
          ...
          <div className="flex gap-3">
            {[{ key: 'is_featured', label: 'Featured' }, { key: 'is_active', label: 'Active' }].map(({ key, label }) => (
              <button key={key} onClick={() => setForm(f => ({ ...f, [key]: !f[key] }))}
                ...
                style={{
                  background: form[key] ? 'rgba(0,245,212,0.12)' : 'var(--cc-bg-0)',
                  color: form[key] ? 'var(--cc-accent-2)' : 'var(--cc-ink-dim)',
                  border: `1px solid ${form[key] ? 'rgba(0,245,212,0.25)' : 'rgba(var(--cc-line-rgb),0.3)'}`,
                }}>
...
export default function AdminParksPanel() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <p className="text-[10px] font-bold tracking-widest" style={{ color: 'var(--cc-accent)' }}>TRUCK PARKS</p>
      ...
      {isLoading ? (
        <div className="flex flex-col gap-3">
          {[1,2,3].map(i => <div key={i} className="h-16 rounded-xl animate-pulse" style={{ background: 'var(--cc-bg-2)' }} />)}
        </div>
      ) : (
            ...
            <div key={park.id} className="flex items-center gap-3 p-3 rounded-2xl"
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  {!park.is_active && <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(var(--cc-warm-red-rgb),0.1)', color: 'var(--cc-warm-red)' }}>INACTIVE</span>}
                  {park.is_sample && <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(251,191,36,0.1)', color: 'var(--cc-amber)' }}>DEMO</span>}
                </div>
              </div>
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <button onClick={() => toggleFeature.mutate({ id: park.id, val: !park.is_featured })}
                  className="w-8 h-8 rounded-lg flex items-center justify-center"
                  style={{ background: park.is_featured ? 'rgba(0,245,212,0.1)' : 'rgba(var(--cc-line-rgb),0.2)' }}>
                  {park.is_featured ? <Star className="w-3.5 h-3.5" style={{ color: 'var(--cc-accent-2)' }} /> : <StarOff className="w-3.5 h-3.5" style={{ color: 'var(--cc-ink-dim)' }} />}
                </button>
                <button onClick={() => toggleActive.mutate({ id: park.id, val: !park.is_active })}
                  className="w-8 h-8 rounded-lg flex items-center justify-center"
                  style={{ background: park.is_active ? 'rgba(0,245,212,0.08)' : 'rgba(var(--cc-line-rgb),0.2)' }}>
                  {park.is_active ? <Eye className="w-3.5 h-3.5" style={{ color: 'var(--cc-accent-2)' }} /> : <EyeOff className="w-3.5 h-3.5" style={{ color: 'var(--cc-ink-dim)' }} />}
                </button>
                <button onClick={() => { setEditing(park); setShowForm(true); }}
                  className="w-8 h-8 rounded-lg flex items-center justify-center"
                  style={{ background: 'rgba(var(--cc-line-rgb),0.2)' }}>
                  <Pencil className="w-3.5 h-3.5" style={{ color: 'var(--cc-ink-dim)' }} />
                </button>
                <button onClick={() => { if (confirm(`Delete "${park.name}"?`)) deletePark.mutate(park.id); }}
                  className="w-8 h-8 rounded-lg flex items-center justify-center"
                  style={{ background: 'rgba(var(--cc-warm-red-rgb),0.08)' }}>
                  <Trash2 className="w-3.5 h-3.5" style={{ color: 'var(--cc-warm-red)' }} />
                </button>
              </div>
            </div>
          ))}
