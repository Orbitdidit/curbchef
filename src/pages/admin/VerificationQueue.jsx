      ...
      <div className="w-full max-w-2xl rounded-3xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-4"
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="text-sm font-bold" style={{ color: 'var(--cc-ink-dim)' }}>✕</button>
          </div>
        </div>
        <div className="p-4 flex items-center justify-center" style={{ minHeight: 300, maxHeight: '70vh', overflow: 'auto' }}>
          {isImage ? (
            ...
            <div className="text-center">
              ...
              <p className="font-semibold mb-3" style={{ color: 'var(--cc-ink)' }}>Document preview</p>
              <a href={url} target="_blank" rel="noopener noreferrer"
...
function DocButton({ url, label, onView }) {
  if (!url) return (
    <span className="text-[10px] font-semibold px-2 py-1 rounded-lg"
      style={{ background: 'var(--cc-bg-2)', color: 'var(--cc-ink-faint)' }}>
      {label}: —
    </span>
  );
  return (
...
function StripePill({ status }) {
  const cfg = {
    payouts_enabled: { label: 'Connected', color: 'var(--cc-green)', bg: 'rgba(34,197,94,0.1)' },
    charges_enabled: { label: 'Charges OK', color: 'var(--cc-amber)', bg: 'rgba(251,191,36,0.1)' },
    onboarding_started: { label: 'Onboarding', color: '#fb923c', bg: 'rgba(251,146,60,0.1)' },
    not_connected: { label: 'Not Connected', color: 'var(--cc-ink-faint)', bg: 'var(--cc-bg-2)' },
      ...
      <div className="p-4 rounded-2xl flex flex-col gap-4"
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] mt-0.5" style={{ color: 'var(--cc-ink-faint)' }}>
      ...
      <div className="px-5 pt-5 max-w-2xl mx-auto">
        {isLoading ? (
          <div className="flex flex-col gap-3">
            {[1,2,3].map(i => <div key={i} className="h-44 rounded-2xl animate-pulse" style={{ background: 'var(--cc-bg-1)' }} />)}
          </div>
        ) : allTrucks.length === 0 ? (
