...
function ParkCard({ park, featured = false }) {
        ...
        <div className="p-3.5">
          <div className="flex items-center gap-3 mb-2.5">
            <div className="flex items-center gap-1.5">
              <Truck className="w-3.5 h-3.5" style={{ color: 'var(--cc-accent-2)' }} />
              <span className="text-xs font-bold" style={{ color: 'var(--cc-ink)' }}>{park.total_trucks || 0} trucks</span>
            </div>
            {park.avg_rating && (
              <div className="flex items-center gap-1">
                <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
                <span className="text-xs font-bold" style={{ color: 'var(--cc-ink)' }}>{park.avg_rating.toFixed(1)}</span>
              </div>
            )}
          </div>
  ...
  return (
    <Link to={`/parks/${park.slug}`} className="flex gap-3 p-3.5 rounded-2xl active:scale-[0.99] transition-transform"
      ...
      <ChevronRight className="w-4 h-4 self-center flex-shrink-0" style={{ color: 'var(--cc-ink-faint)' }} />
    </Link>
  );
}
      ...
      <div className="px-4">
        ...
        {isLoading ? (
          <div className="flex flex-col gap-3">
            {[1,2,3].map(i => <div key={i} className="h-24 rounded-2xl animate-pulse" style={{ background: 'var(--cc-surface)' }} />)}
          </div>
        ) : (
          <div className="flex flex-col gap-3">
