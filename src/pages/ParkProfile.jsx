  ...
  if (!park) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--cc-black)' }}>
        <div className="w-7 h-7 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--cc-accent-2) transparent transparent transparent' }} />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pb-28" style={{ background: 'var(--cc-black)' }}>
      ...
      <div className="px-4 pt-4 pb-3">
        ...
        {park.description && (
          <p className="text-sm mt-3 leading-relaxed" style={{ color: 'var(--cc-ink-muted)' }}>{park.description}</p>
        )}
      </div>
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="p-4 rounded-2xl" style={{ background: 'var(--cc-surface)' }}>
                ...
                {Object.entries(park.operating_hours).map(([day, hours]) => (
                  <div key={day} className="flex justify-between py-1.5"
                    ...
                    <span className="text-sm font-semibold capitalize" style={{ color: 'var(--cc-ink-muted)' }}>{day}</span>
                    <span className="text-sm font-semibold" style={{ color: 'var(--cc-ink)' }}>{hours}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="p-4 rounded-2xl flex flex-col gap-3" style={{ background: 'var(--cc-surface)' }}>
              ...
              <a href={`https://maps.google.com/?q=${encodeURIComponent(park.address)}`}
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: 'var(--cc-ink)' }}>Get Directions</p>
                  <p className="text-xs" style={{ color: 'var(--cc-ink-muted)' }}>{park.address}</p>
                </div>
              </a>
              {park.phone && (
                <a href={`tel:${park.phone}`} className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: 'rgba(0,245,212,0.1)' }}>
                    <Phone className="w-4 h-4" style={{ color: 'var(--cc-accent-2)' }} />
                  </div>
                  <p className="text-sm font-semibold" style={{ color: 'var(--cc-ink)' }}>{park.phone}</p>
                </a>
              )}
              {park.website && (
                <a href={park.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: 'rgba(0,245,212,0.1)' }}>
                    <ExternalLink className="w-4 h-4" style={{ color: 'var(--cc-accent-2)' }} />
                  </div>
                  <p className="text-sm font-semibold" style={{ color: 'var(--cc-ink)' }}>Website</p>
                </a>
              )}
              {park.instagram && (
                <a href={`https://instagram.com/${park.instagram.replace('@','')}`}
                  ...
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: 'rgba(0,245,212,0.1)' }}>
                    <Instagram className="w-4 h-4" style={{ color: 'var(--cc-accent-2)' }} />
                  </div>
                  <p className="text-sm font-semibold" style={{ color: 'var(--cc-ink)' }}>{park.instagram}</p>
                </a>
              )}
            </div>
          </div>
