...
export default function OrderDetail() {
  ...
  if (!order) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--cc-black)' }}>
        <div className="w-8 h-8 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: 'var(--cc-accent-2) transparent transparent transparent' }} />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen dot-bg" style={{ background: 'var(--cc-black)' }}>
      {/* Header */}
      ...
      <div className="px-5 pb-32">
                ...
                <div className="w-6 h-6 rounded-full flex items-center justify-center transition-all"
                  ...
                  {(done || active) && (
                    <svg className="w-3 h-3" viewBox="0 0 12 12" fill="none">
                      <path d="M2 6l3 3 5-5" stroke="var(--cc-black)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  )}
                </div>
        ...
        {/* Truck info */}
        <div className="p-4 rounded-2xl flex items-center gap-3 mb-5" style={{ background: 'var(--cc-surface)' }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(var(--cc-accent-rgb),0.1)' }}>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--cc-surface)' }}>
            ...
            {order.tax > 0 && (
              <div className="flex justify-between px-4 py-2">
                <span className="text-xs" style={{ color: 'var(--cc-ink-muted)' }}>Tax (8.25%)</span>
                <span className="text-xs" style={{ color: 'var(--cc-ink-muted)' }}>${order.tax?.toFixed(2)}</span>
              </div>
            )}
            <div className="flex justify-between px-4 py-2">
              <span className="text-xs" style={{ color: 'var(--cc-ink-muted)' }}>Service Fee</span>
              <span className="text-xs" style={{ color: 'var(--cc-ink-muted)' }}>$1.50</span>
            </div>
            <div className="flex justify-between px-4 py-2">
              <span className="text-xs" style={{ color: 'var(--cc-ink-muted)' }}>Tip</span>
            ...
            <div className="flex flex-col gap-3 mb-4">
              {truck?.phone ? (
                <a href={`tel:${truck.phone}`}
                  ...
                  <div>
                    <p className="font-bold text-sm" style={{ color: 'var(--cc-cream)' }}>Call Truck</p>
                    <p className="text-xs" style={{ color: 'var(--cc-ink-muted)' }}>{truck.phone}</p>
                  </div>
                </a>
              ) : (
